...
function createOtherObjects()
{
  ...
  titleMessage.text = "Gothic Tower";
  titleMessage.visible = false;
...
{ 
  ...
  for (var i=0;i<waters.length;i++){
    if (hitTestRectangle(cat,waters[i])){
  ...
  for (var i=0;i<flames.length;i++){
    ...
    for (var j=0;j<boxes.length;j++){
      if (hitTestRectangle(flame,boxes[j])){
        removeObject(flame, flames);
        removeObject(flame, sprites);
        break;
      }
  ...
  for (var i=0;i<bloods.length;i++){
    if (bloods[i].moving){
      for (var j=0;j<boxes.length;j++){
        if (hitTestRectangle(bloods[i],boxes[j])){
  ...
  for (var i=0;i<shots.length;i++){
    var shot=shots[i];
    for (var j=0;j<boxes.length;j++){
      if (hitTestRectangle(shot,boxes[j])){
        removeObject(shot, shots);
        removeObject(shot, sprites);
        break;
      }
  ...
  function monsterSplat(monsters){
    for (var i=0;i<shots.length;i++){
      ...
      for (var j=0;j<monsters.length;j++){
        ...
        if (hitTestRectangle(shot,monster) && monster.visible){
          removeObject(shot, shots);
          removeObject(shot, sprites);
  ...
  for (var i=0;i<coins.length;i++){
    var coin = coins[i];
    if (hitTestRectangle(cat,coin) && coin.visible){
      coin.visible=false;
      score++;
